...
from src.smart_excel_utils import ExcelHandler

def render():
    st.header("📊 Smart Excel Export")
    st.markdown("### Interactive Threshold & Coloring Analysis")
    st.info("Upload an Excel file to detect and apply conditional coloring rules based on threshold rows.")

    # Init Handler
    handler = ExcelHandler()

    # Init Session State for Overrides
    if 'smart_manual_overrides' not in st.session_state:
        st.session_state.smart_manual_overrides = {} # {(row_idx, col_idx): 'HEX'}

    # 1. Upload
    uploaded_file = st.file_uploader("Upload Excel File", type=["xlsx"], key="smart_uploader")
    ...
    if uploaded_file:
        ...
        with col1:
            ...
            selected_sheet = st.selectbox("Select Sheet to Analyze", sheet_names, key="smart_sheet_select")
            ...
            def render_interactive_analysis():
                ...
                with col_sel:
                    ...
                    target_cols_indices = st.multiselect(
                        ...
                        key="smart_col_select"
                    )
                    
                    # Clear overrides if sheet changes
                    if "smart_last_sheet" not in st.session_state:
                        st.session_state.smart_last_sheet = selected_sheet
                    elif st.session_state.smart_last_sheet != selected_sheet:
                        st.session_state.smart_manual_overrides = {}
                        st.session_state.smart_active_row_idx = None
                        st.session_state.smart_last_sheet = selected_sheet
                ...
                with col_view:
                    ...
                    use_full_view = st.checkbox("Show All Rows (May be slow)", value=False, key="smart_full_view")
                    ...
                    with st.expander("🛠️ Debug Interactive State (Global)", expanded=False):
                        st.write(f"Active Index: {st.session_state.get('smart_active_row_idx')}")
                        st.write(f"Overrides Count: {len(st.session_state.smart_manual_overrides)}")
                    
                    if target_cols_indices and threshold_rows_input:
                        try:
                            col_prev_params, _ = st.columns([1,3])
                            with col_prev_params:
                                 preview_limit = st.number_input("Preview Rows Limit", min_value=10, max_value=5000, value=100, step=50, key="smart_prev_limit")
                            ...
                            for rel_idx, r in enumerate(prev_rows):
                                ...
                                for k, v in r.items():
                                     ...
                                     if (curr_zero_based_row_idx, col_idx) in st.session_state.smart_manual_overrides:
                                         ov_hex = st.session_state.smart_manual_overrides[(curr_zero_based_row_idx, col_idx)]
                                         if ov_hex: bg = ov_hex
                                         else: bg = None 
                                     d[k] = val
                                prev_data.append(d)
                            ...
                            def preview_styler(row):
                                ...
                                if 0 <= list_idx < len(prev_rows):
                                    ...
                                    for col_name in df_prev_show.columns: 
                                        ...
                                        if (zero_based, c_idx) in st.session_state.smart_manual_overrides:
                                            ov = st.session_state.smart_manual_overrides[(zero_based, c_idx)]
                            ...
                            if selection and "selection" in selection and "rows" in selection["selection"]:
                                ...
                                if rows_sel:
                                    ...
                                    if row_pos < len(df_prev_show):
                                        ...
                                        if st.session_state.get('smart_active_row_idx') != excel_row_num:
                                             st.session_state.smart_active_row_idx = excel_row_num
                                             st.rerun() # Explicit rerun to show Edit Panel immediately
                                else:
                                    if 'smart_active_row_idx' in st.session_state:
                                        del st.session_state.smart_active_row_idx
                                        st.rerun()
    
                            # --- INTERACTIVE CORRECTION PANEL ---
                            if 'smart_active_row_idx' in st.session_state:
                                 excel_row_num = st.session_state.smart_active_row_idx
                                 logic_row_idx = excel_row_num - 1
                                 st.info(f"Editing Row: {excel_row_num}")
                                 
                                 if st.button("Close / Deselect Row", key="smart_close_row_edit"):
                                     del st.session_state.smart_active_row_idx
                                     st.rerun()
                                 ...
                                 for i, col_idx in enumerate(target_cols_indices):
                                    ...
                                    curr_bg = None
                                    if (logic_row_idx, col_idx) in st.session_state.smart_manual_overrides:
                                        curr_bg = st.session_state.smart_manual_overrides[(logic_row_idx, col_idx)]
                                    else:
                                    ...
                                    with cols[i]:
                                        ...
                                        if st.button(f"Cycle Color", key=f"smart_btn_{logic_row_idx}_{col_idx}"):
                                            st.session_state.smart_manual_overrides[(logic_row_idx, col_idx)] = next_color
                                            st.rerun()
                    ...
                    # 6. Action: Apply
                    if st.button("Apply Threshold Coloring", type="primary", key="smart_apply_btn"):
                        if not target_cols_indices:
                            ...
                            with st.spinner("Processing..."):
                                ...
                                # Using standard temp path might be better?
                                # handler.apply_thresholds writes to out_path.
                                ...
                                handler.apply_thresholds(
                                    ...
                                    data_start_row=None,
                                    header_row_idx=header_row_idx,
                                    manual_overrides=st.session_state.smart_manual_overrides
                                )
