...
<div id="compass" style="position:absolute; bottom:10px; left:10px; z-index:9999; width:80px; height:80px; cursor:pointer;" title="Click to reset rotation">
    <div id="compassInner" style="position:relative; width:100%; height:100%; background:rgba(0,0,0,0.6); border-radius:50%; border:3px solid white; box-shadow:0 2px 8px rgba(0,0,0,0.3); transition: transform 0.3s ease;">
        ...
        <div style="position:absolute; top:8px; left:50%; transform:translateX(-50%); border-left: 12px solid transparent; border-right: 12px solid transparent; border-bottom: 24px solid #ff3333;"></div>
        <div style="position:absolute; top:-8px; left:50%; transform:translateX(-50%); color:white; font-weight:bold; font-size:16px; text-shadow:2px 2px 4px black;">N</div>
    </div>
</div>
...
<div id="map-legend" style="position:absolute; bottom:10px; right:10px; z-index:9999; background:rgba(255,255,255,0.95); padding:10px; border-radius:6px; font-family:Arial,Helvetica,sans-serif; font-size:12px; box-shadow:0 0 5px rgba(0,0,0,0.2);">
  ...
  <div style="display:flex; align-items:center; margin-bottom:10px;">
    ...
    <span>Excel Points (Draggable)</span>
  </div>
...
(() => {{
  ...
  function initDots() {{
      ...
      // 2. Add Excel Points (Orange - Draggable/Moveable)
      orangePoints.forEach(pt => {{
          ...
          marker.bindPopup(`<b>Excel Point (Draggable)</b><br>ID: ${{pt.id}}<br>Lat: ${{pt.lat.toFixed(6)}}<br>Lon: ${{pt.lon.toFixed(6)}}`);
          
          // Update popup on drag
          marker.on('drag', function(e) {{
              const latlng = e.target.getLatLng();
              marker.setPopupContent(`<b>Excel Point (Dragging...)</b><br>ID: ${{pt.id}}<br>Lat: ${{latlng.lat.toFixed(6)}}<br>Lon: ${{latlng.lng.toFixed(6)}}`);
          }});
          
          marker.on('dragend', function(e) {{
              const latlng = e.target.getLatLng();
              marker.setPopupContent(`<b>Excel Point (Draggable)</b><br>ID: ${{pt.id}}<br>Lat: ${{latlng.lat.toFixed(6)}}<br>Lon: ${{latlng.lng.toFixed(6)}}`);
              console.log(`Excel Point ${{pt.id}} moved to: [${{latlng.lat}}, ${{latlng.lng}}]`);
          }});
      }});
  }}
